        ...
        public ValidationFailure Test(NotifiarrSettings settings)
        {
            ...
            catch (HttpException ex)
            {
                switch ((int)ex.Response.StatusCode)
                {
                    ...
                    case 400:
                        _logger.Error(ex, "Unable to send test message. Ensure Lidarr Integration is enabled & assigned a channel on Notifiarr");
                        return new ValidationFailure("", "Unable to send test message. Ensure Lidarr Integration is enabled & assigned a channel on Notifiarr");
                    case 520:
                    case 521:
                    case 522:
                    case 523:
        ...
        private void ProcessNotification(StringDictionary message, NotifiarrSettings settings)
        {
            ...
            catch (HttpException ex)
            {
                switch ((int)ex.Response.StatusCode)
                {
                    ...
                    case 400:
                        _logger.Error(ex, "Unable to send notification. Ensure Lidarr Integration is enabled & assigned a channel on Notifiarr");
                        throw new NotifiarrException("Unable to send notification. Ensure Lidarr Integration is enabled & assigned a channel on Notifiarr", ex);
                    case 520:
                    case 521:
                    case 522:
                    case 523:
